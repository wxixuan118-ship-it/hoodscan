...
import { getNetworkSnapshot, getLatestBlocks, formatGwei } from '@/lib/robinhood-rpc';

export const dynamic = 'force-dynamic';

export const metadata: Metadata = {
  title: 'Gas Tracker | Robinhood Chain',
  description: 'Real-time gas prices on Robinhood Chain. Track slow, standard and fast gas tiers and view historical gas usage per block.',
  alternates: { canonical: 'https://www.hood-chain.com/gas-tracker' },
};
...
export default async function GasTrackerPage() {
  ...
  // Parse base gas price in Gwei
  const baseGwei = snapshot
    ? parseFloat(snapshot.gasPrice.replace(' Gwei', ''))
    : null;

  // Compute avg gas used % and avg gas price from recent blocks
  ...
  const jsonLd = {
    ...
    '@type': 'WebPage',
    name: 'Gas Tracker — Robinhood Chain',
    url: 'https://www.hood-chain.com/gas-tracker',
    description: 'Live gas price tracker for Robinhood Chain (Chain ID 4663).',
  };
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, margin: '0 0 0.5rem', letterSpacing: '-0.5px' }}>Gas Tracker</h1>
        <p style={{ color: 'var(--muted)', fontSize: '0.9rem', margin: 0 }}>
          Real-time gas prices for Robinhood Chain · Updated on every page load
        </p>
      </div>
      ...
      {/* Gas usage per block — horizontal bar chart */}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '1.5rem' }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: '0.875rem', lineHeight: 1.7, margin: 0 }}>
          The base gas price shown reflects the current <code>eth_gasPrice</code> RPC value.
          Slow / Standard / Fast tiers are estimated multipliers (1×, 1.2×, 1.5×) — on a low-congestion L2,
          even the "slow" tier confirms quickly.
        </p>
      </div>
    </div>
  );
